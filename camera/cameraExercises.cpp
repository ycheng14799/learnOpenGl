...
glm::mat4 getLookAt(glm::vec3 positionVec, glm::vec3 targetVec, glm::vec3 upVec); 
...
int main()
{
  ...
  while(!glfwWindowShouldClose(window)) {
    ...
    glm::mat4 view;
    // Set lookAt 
    view = getLookAt(cameraPos, cameraPos + cameraFront, cameraUp); 
...
void processInput(GLFWwindow *window)
{
	...
	// Define movement in xy plane instead of xyz 
	glm::vec3 moveFront = glm::normalize(glm::vec3(cameraFront.x, 0, cameraFront.z));
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) 
		cameraPos += cameraSpeed * moveFront; 
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
		cameraPos -= cameraSpeed * moveFront; 
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
		cameraPos -= glm::normalize(glm::cross(moveFront, cameraUp)) * cameraSpeed; 
...
glm::mat4 getLookAt(glm::vec3 positionVec, glm::vec3 targetVec, glm::vec3 upVec) { 
	...
	glm::vec3 directionVec = glm::normalize(positionVec - targetVec); 
	// Define right direction
	glm::vec3 rightVec = glm::normalize(glm::cross(glm::normalize(upVec), directionVec));
	// Define up for camera 
	glm:: vec3 cameraUpVec = glm::cross(directionVec, rightVec); 
	lookAtMat[0][0] = rightVec.x;
	lookAtMat[1][0] = rightVec.y;
	lookAtMat[2][0] = rightVec.z;
	lookAtMat[0][1] = cameraUpVec.x;
	lookAtMat[1][1] = cameraUpVec.y;
	lookAtMat[2][1] = cameraUpVec.z;
	lookAtMat[0][2] = directionVec.x;
	lookAtMat[1][2] = directionVec.y;
	lookAtMat[2][2] = directionVec.z; 
	glm::mat4 positionalComponent(1.0f); 
	positionalComponent[3][0] = -positionVec.x;
	positionalComponent[3][1] = -positionVec.y;
	positionalComponent[3][2] = -positionVec.z; 
	lookAtMat = lookAtMat * positionalComponent; 
	return lookAtMat;
}		
